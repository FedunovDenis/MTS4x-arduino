...
- Внутренний диапазон измерений чипа: примерно **−103…+153 °C**
...
- Возможности: CRC, пороги **TH/TL**, `Alert_Mode`, heater, user-регистры, EEPROM, паразитное питание
- Платформы: ESP8266, ESP32, AVR (Uno/Nano/Mega) и другие Arduino-совместимые платы
- Примеры:
  - Полный Serial-демо: `MTS4x_FullDemo`
  - Wi-Fi-метеостанция с максимальной точностью, JSON-API и отправкой на **narodmon.ru**: `MTS4x_MeteoStation`
...
14. [Работа с EEPROM, порогами и аварийными сигналами](#работа-с-eeprom-порогами-и-аварийными-сигналами)  
15. [Паразитное питание](#паразитное-питание)  
16. [Changelog](#changelog)  
17. [License](#license)  
18. [English summary (short)](#english-summary-short)  
...
- **MTS4**: ±0.1 °C в узком диапазоне комнатной температуры (смотрите даташит)  
...
### Аппаратное усреднение (AVG)

Чип поддерживает аппаратное усреднение:

- `AVG_1`, `AVG_8`, `AVG_16`, `AVG_32`

Дополнительно можно делать цифровое усреднение в микроконтроллере  
(пример — метеостанция `MTS4x_MeteoStation`).

### Режимы измерения

- **Single-shot** (одиночное измерение)  
  Чип спит между измерениями, каждое измерение запускается отдельной командой.  
...
- температуры: байты `Temp_Lsb / Temp_Msb + Crc_temp`;
...
|--------|--------------------------------------|------------------------------|----------------|
| MTS4   | узкое окно для комнатной зоны    | до ±0.1 °C                   | 0x41           |
| MTS4Z  | от 0 до +50 °C                       | до ±0.1 °C                   | 0x41           |
| MTS4P  | от −25 до +25 °C                     | до ±0.1 °C                   | 0x41           |
| MTS4B  | широкий диапазон                     | до ±0.5 °C                   | 0x41           |

Особенно удобен **MTS4P+T4** — готовый проводной датчик для быстрого подключения к ESP8266/ESP32.
